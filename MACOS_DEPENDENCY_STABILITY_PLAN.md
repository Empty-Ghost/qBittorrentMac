...
- The macOS sleep inhibitor now owns an explicit valid/invalid assertion state, ignores repeated transitions, retries failed releases, and releases an active assertion during destruction. Deterministic platform-call tests cover creation failure, invalid IDs, release failure, repeated calls, and destruction; replacing the periodic torrent scan and completing live sleep/`pmset` validation remain pending.
